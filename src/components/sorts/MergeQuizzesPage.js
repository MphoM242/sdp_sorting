...
import { getAuth, onAuthStateChanged, onIdTokenChanged,signInWithEmailAndPassword } from "firebase/auth";
...
const  MergeQuizzesPage= () => {
    ...
    const fetchQuizzes = async () => {
      onAuthStateChanged(auth, async (user) => {
        if (user) {
          ...
          // Continue with Firebase queries
  ...
  useEffect(() => {
    const fetchQuizQuestions = async (QuizID) => {
      const demoQuestions = collection(db, 'Sorts', 'Merge', QuizID.toString());
    ...
    onAuthStateChanged(auth, (user) => {
      if (user) {
        ...
        if (selectedQuiz) {
          ...
          fetchQuizQuestions(selectedQuiz.quizId);
        }
      } else {
        // User is signed out
        console.log("User is logged out");
